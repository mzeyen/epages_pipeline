...
def build(String branch) {
  ...
  stage("Build ${branch}") {
    try {
      sh """
        chmod +x gradlew
        docker run --rm -v "\$(pwd):/opt/esf-epages-unity" -t "epages/oracle-jdk8-fontconfig-freetype:latest" \\
        /bin/bash -c 'cd /opt/esf-epages-unity; ./gradlew clean dist;'
        docker build -t epages/esf-epages-unity:${branch} .
      """
    } finally {
...
def testAll(String branch) {
  ...
  stage("Test all") {
    try {
      ...
      sh """
        docker run --rm --user=root --env DBUS_SESSION_BUS_ADDRESS=/dev/null \\
        -v "\$(pwd)/log:/home/esf/esf/log" \\
        -t "epages/esf-epages-unity:${branch}" \\
        -browser CHROME \\
        -sub esf-epages-unity \\
        -groups UNITY \\
        -email p.domin@epages.com \\
        -shop ${shopName} \\
        -url https://greenteam-vm-01.intern.epages.de/epages \\
        -surl https://${shopName}.greenteam-vm-01.intern.epages.de \\
        -ap 0 \\
        -sp \\
        -l de_DE \\
        -r;
  ...
  stage("Publish") {
    withCredentials([[
      ...
      usernameVariable: 'USER']]) {
        sh """
          docker tag epages/esf-epages-unity:${branch} epages/esf-epages-unity:latest
