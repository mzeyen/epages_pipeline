...
@groovy.transform.Field infraDir = "/root/epages-infra"
@groovy.transform.Field esfTestShop = "DemoShop${env.BUILD_NUMBER}"
@groovy.transform.Field epRoot = "/srv/epages/eproot"
...
def updateSources() {
  ...
  stage("CONFIG: Change ePages sources") {
...
def configAppServers() {
  ...
  stage("CONFIG: Reconfigure app servers, stop cron jobs") {
    withEnv(["BUILD_ID=dontKillMe"]) {
...
def reinstallEpages(String additionalCartridges) {
  ...
  String stageName = "TEST: Reinstall ePages"
  stage(stageName) {
    withEnv(["BUILD_ID=dontKillMe"]) {
      try {
...
def reinstallEpagesMultistore() {
  ...
  String stageName = "TEST Reinstall ePages Multistore"
  stage(stageName) {
    withEnv(["BUILD_ID=dontKillMe"]) {    
      try {
...
def perlModuleTests(String changedCartridges, String[] stores = ["Store"]) {
  ...
  String stageName = "TEST: Integration tests."
  stage(stageName) {
    env.SKIP_TIMING = "yes"
...
/**
 * Check the unit tests of all cartridges.
 * 
 * @param stores array of stores to test.
 *   default: ["Store"].
 */
def checkUnitTests(String[] stores = ["Store"]) {

  String stageName = "TEST: All unit tests"
  stage(stageName) {
    env.SKIP_TIMING = "yes"
    env.SKIP_EXTERNAL_TESTS = "yes" 
    try {
      // Only test cartridges with a "t/unit" folder.
      sh """
        source /etc/default/epages6
        for dir in \$EPAGES_CARTRIDGES/DE_EPAGES/* ; do
          if [ -d $dir/t/unit ] ; then
            cd $dir
            $PERL Makefile.PL
            make unit_test
          fi
...
/**
 * Check all tests of the cartridges.
 *
 * @param stores array of stores to test.
 *   default: ["Store"].
 */
def checkTestCartridgeTests(String[] stores = ["Store"]) {
  
  String stageName = "TEST: Test Cartridge tests"
  stage(stageName) {
    env.SKIP_TIMING = "yes"
    env.SKIP_EXTERNAL_TESTS = "yes" 
    try {
...
def runEsfTests(String store = "Store") {
  ...
  String stageName = "TEST: ESF smoke tests"
  stage(stageName) {
    try {
      sh "${infraDir}/scripts/epages/create_shop.sh -n ${esfTestShop} -t 'Demo' -s ${store} -i"
      build job: "Run_ESF_tests", 
            parameters: [string(name: 'TARGET_DOMAIN', value: getCurrentHostname()),
                         string(name: 'SHOP', value: esfTestShop),
                         string(name: 'TESTGROUPS', value: 'SMOKETEST'),
                         string(name: 'STORE', value: store)]
    } catch (e) {
      this.sendMailToCommitter(stageName)
      throw e;
    }
...
def runTestCartridgeTests(String store, String rootPath) {
  ...
  sh """
    export EPAGES_TESTSITE=${store}
    cd ${rootPath}/Cartridges/DE_EPAGES
    ${rootPath}/Perl/bin/perl Test/t/translationFiles.t
    ${rootPath}/Perl/bin/perl Test/t/translationJS.t
    ${rootPath}/Perl/bin/perl Test/t/pageTypes.t
    ${rootPath}/Perl/bin/perl Test/t/ObjectReferenceTest.t
  """
}
