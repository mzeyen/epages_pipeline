  ...
  stage('Clone repo from branch') {
    sh """
        if [[ ! -d ~/$repository/.git ]] ; then
            cd ~
            git clone -b ${branch} git@github.com:$remote/${repository}.git ${repository}
        else
            cd ~/$repository && git pull origin $branch
        fi
    """
  }
}
...
/**
 * Clone Cartridges git repository and use a given branch.
 *
 * @params remote The remote, from which should be cloned.
 * @params repository The repository which should be cloned.
 * @params branch The branch, which should be cloned.
 */
def cloneCartridgesFromBranch(String remote, String repository, String branch) {

    stage('Clone repo from branch') {
        sh """
        if [[ ! -d /srv/epages/eproot/$repository/.git ]] ; then
            cd /srv/epages/eproot/
            git clone -b ${branch} git@github.com:$remote/${repository}.git ${repository}
        else
            cd /srv/epages/eproot/$repository && git pull origin $branch
        fi
    """
    }
}
